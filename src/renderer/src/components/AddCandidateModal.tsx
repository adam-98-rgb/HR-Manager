...
export default function AddCandidateModal({ isOpen, onClose, onAdd, defaultCompany }: AddCandidateModalProps): JSX.Element {
    const [formData, setFormData] = useState<any>({
        id: '', // Will be used as temp ID if needed, or generated by DB/Logic
        nom: '',
        prenom: '',
        dateNaissance: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        // Generate a temp ID if not present, useful for key prop or file storage before DB
        const fullName = `${formData.nom} ${formData.prenom}`.trim()
        const candidateData = {
            ...formData,
            fullName,
            id: formData.id || `cand_${Date.now()}`
        }
        onAdd(candidateData)
        onClose()
    }
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    ...
                    >
                        <div className="p-8 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h2 className="text-2xl font-bold text-white font-['Outfit'] tracking-tight mb-1">Add Candidate</h2>
                                <p className="text-[#ffcc4d] font-medium text-xs">New recruitment process</p>
                            </div>
                            <button
                                onClick={onClose}
                        ...
                        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                ...
                                <div className="space-y-4">
                                    <label className={labelClasses}>Salaire Net</label>
                                    <input
                                        type="text"
                                        className={inputClasses}
                                ...
                                <div className="space-y-4">
                                    <label className={labelClasses}>Date Embauche (Predicted)</label>
                                    <input
                                        type="date"
                                        className={inputClasses}
                                        style={{ colorScheme: 'dark' }}
